  ...
  if (req.query.query === "deletemany") {
    try {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          details: `Deleted customers with IDs: ${req.body.map((id) => id).join(', ')}`,
        },
      });
    ...
    try {
      ...
      const createdCustomer = await prisma.customer.createMany({
        data: req.body.map((customer) => {
          return {
            name: customer.name,
            nameresponsable: customer.nameresponsable,
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'CREATE_MANY_CUSTOMERS',
          details: `Created customers: ${JSON.stringify(req.body)}`,
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'CREATE_SINGLE_CUSTOMER',
          entityId: createdCustomer.id,
          entityType: 'customer',
          details: `Created customer: ${JSON.stringify(req.body)}`,
        },
      });
  ...
  if (req.query.query === "all") {
    try {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          details: `Retrieved all customers.`,
        },
      });
    ...
    // Journalisation
      await prisma.auditLog.create({
        data: {
          ...
          details: `Retrieved customer info.`,
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_ALL_INACTIVE_CUSTOMERS',
          details: `Retrieved all inactive customers.`,
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_ALL_ACTIVE_CUSTOMERS',
          details: `Retrieved all active customers.`,
        },
      });
...
const getSingleCustomer = async (req, res) => {
  try {
    ...
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub),
        action: 'GET_SINGLE_CUSTOMER',
        entityId: singleCustomer.id,
        entityType: 'customer',
        details: `Retrieved customer with ID ${singleCustomer.id}.`,
      },
    });
  ...
  try {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        entityType: 'customer',
        details: `Updated customer with ID ${updatedCustomer.id}.`,
      },
    });
...
const deleteSingleCustomer = async (req, res) => {
  try {
    ...
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub),
        action: 'DELETE_SINGLE_CUSTOMER',
        entityId: deletedCustomer.id,
        entityType: 'customer',
        details: `Deleted customer with ID ${deletedCustomer.id}.`,
      },
    });
