  ...
  try {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        action: 'CREATE_RETURN_SALE_INVOICE',
        entityId: createdReturnSaleInvoice.id,
        entityType: 'returnSaleInvoice',
        details: `Created return sale invoice with ID ${createdReturnSaleInvoice.id} for sale invoice ID ${req.body.saleInvoice_id}.`,
      },
    });
...
const getAllReturnSaleInvoice = async (req, res) => {
  if (req.query.query === "info") {
    ...
    await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_RETURN_SALE_INVOICE_INFO',
          entityType: 'returnSaleInvoice',
          details: 'Fetched return sale invoice info.',
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_ALL_RETURN_SALE_INVOICES',
          entityType: 'returnSaleInvoice',
          details: 'Fetched all return sale invoices.',
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_RETURN_SALE_INVOICE_GROUP',
          entityType: 'returnSaleInvoice',
          details: 'Fetched grouped return sale invoices.',
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_RETURN_SALE_INVOICES_BY_STATUS_FALSE',
          entityType: 'returnSaleInvoice',
          details: `Fetched return sale invoices with status false from ${req.query.startdate} to ${req.query.enddate}.`,
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_RETURN_SALE_INVOICES_BY_STATUS_TRUE',
          entityType: 'returnSaleInvoice',
          details: `Fetched return sale invoices with status true from ${req.query.startdate} to ${req.query.enddate}.`,
        },
      });
...
const getSingleReturnSaleInvoice = async (req, res) => {
  try {
    ...
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub),
        action: 'GET_SINGLE_RETURN_SALE_INVOICE',
        entityId: req.params.id,
        entityType: 'returnSaleInvoice',
        details: `Fetched return sale invoice with ID ${req.params.id}.`,
      },
    });
...
const updateSingleReturnSaleInvoice = async (req, res) => {
  try {
    ...
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub),
        action: 'UPDATE_RETURN_SALE_INVOICE',
        entityId: req.params.id,
        entityType: 'returnSaleInvoice',
        details: `Updated return sale invoice with ID ${req.params.id}.`,
      },
    });
...
const deleteSingleReturnSaleInvoice = async (req, res) => {
  try {
    ...
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub),
        action: 'DELETE_RETURN_SALE_INVOICE',
        entityId: req.params.id,
        entityType: 'returnSaleInvoice',
        details: `Deleted return sale invoice with ID ${req.params.id}.`,
      },
    });
