  ...
  try {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        details: `Created Sale Invoice #${createdInvoice.id}. Total Sale Price: ${totalSalePrice}, Total Purchase Price: ${totalPurchasePrice}, Discount: ${req.body.discount}, Paid Amount: ${req.body.paid_amount}, Due Amount: ${due_amount}.`,
      },
    });
...
const getAllSaleInvoice = async (req, res) => {
  if (req.query.query === "info") {
    ...
    await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub), // L'ID de l'utilisateur qui a effectué l'action
          action: 'GET_ALL_SALE_INVOICE_INFO',
          details: `Retrieved aggregations for sale invoices.`,
        },
      });
    ...
    try {
          ...
          [aggregations, saleInvoices] = await prisma.$transaction([
            ...
            prisma.saleInvoice.findMany({
              ...
              where: {
                ...
                // Ajout du filtre sur paid_amount strictement supérieur à 0
                paid_amount: {
                  gt: 0,
                },
              },
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub), // L'ID de l'utilisateur qui a effectué l'action
          action: 'GET_ALL_SALE_INVOICE',
          details: `Retrieved all sale invoices with paid_amount > 0. Number of Sale Invoices: ${saleInvoices.length}.`,
        },
      });
...
const getSingleSaleInvoice = async (req, res) => {
  try {
    ...
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub), // L'ID de l'utilisateur qui a effectué l'action
        action: 'GET_SINGLE_SALE_INVOICE',
        entityId: singleSaleInvoice.id,
        entityType: 'saleInvoice',
        details: `Retrieved Sale Invoice #${singleSaleInvoice.id}.`,
      },
    });
