...
const getDashboardData = async (req, res) => {
  try {
    ...
    const formattedData4 = [
      { type: "ventes X", value: Number(salesInfoSpa._sum.total_amount) },
    ];
    const formattedData5 = [
      {
        type: "ventes Y",
        value: Number(salesInfoBtq._sum.total_amount),
      },
    ];
    ...
    const productInfo = await prisma.product.aggregate({
      _count: {
        id: true,
      },
    });
    const getProductMultiplications = async () => {
      const products = await prisma.product.findMany();

      const multiplications = products.map((product) => {
        return product.purchase_price * product.quantity;
      });

      const totalMultiplication = multiplications.reduce(
        (acc, curr) => acc + curr,
        0
      );
	...
	const productTotal = await getProductMultiplications();
    // concat 2 object
    const cardInfo = {
      ...
      product_count: productInfo._count.id,
      product_total: Number(productTotal),
    };
    res.json({
      saleProfitCount,
      SupplierVSCustomer,
