...
const createSinglePurchaseInvoice = async (req, res) => {
  ...
  try {
    ...
    // Log the create purchase invoice action
    await prisma.auditLog.create({
      data: {
        ...
        details: `Created purchase invoice with ID: ${createdInvoice.id}`,
      },
    });
...
const getAllPurchaseInvoice = async (req, res) => {
  if (req.query.query === "info") {
    ...
    // Log the get purchase invoice info action
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub),
        action: 'GET_PURCHASE_INVOICE_INFO',
        details: 'Retrieved purchase invoice info',
      },
    });
    ...
    try {
      ...
      // Log the get all purchase invoices action
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_ALL_PURCHASE_INVOICES',
          details: `Retrieved all purchase invoices from ${new Date(req.query.startdate).toLocaleDateString()} to ${new Date(req.query.enddate).toLocaleDateString()}`,
...
const getSinglePurchaseInvoice = async (req, res) => {
  try {
    ...
    // Log the get single purchase invoice action
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub),
        action: 'GET_SINGLE_PURCHASE_INVOICE',
        entityId: Number(req.params.id),
        entityType: 'purchaseInvoice',
        details: `Retrieved purchase invoice with ID: ${req.params.id}`,
      },
    });
