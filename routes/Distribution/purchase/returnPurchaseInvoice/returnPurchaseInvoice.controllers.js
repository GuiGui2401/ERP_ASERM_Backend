...
const createSingleReturnPurchaseInvoice = async (req, res) => {
  ...
  try {
    ...
    // Log the create single return purchase invoice action
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'CREATE_RETURN_PURCHASE_INVOICE',
          details: `Created return purchase invoice with ID ${createdReturnPurchaseInvoice.id} for purchase invoice ID ${req.body.purchaseInvoice_id}`,
        },
      });
  ...
  if (req.query.query === "info") {
    ...
    await prisma.auditLog.create({
        data: {
          ...
          action: 'GET_ALL_RETURN_PURCHASE_INVOICE',
          details: `Aggregated return purchase invoice info retrieved.`,
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub), // L'ID de l'utilisateur qui a effectué l'action
          action: 'GET_ALL_RETURN_PURCHASE_INVOICE',
          details: `All return purchase invoices retrieved.`,
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub), // L'ID de l'utilisateur qui a effectué l'action
          action: 'GET_ALL_RETURN_PURCHASE_INVOICE',
          details: `Grouped return purchase invoices retrieved.`,
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub), // L'ID de l'utilisateur qui a effectué l'action
          action: 'GET_ALL_RETURN_PURCHASE_INVOICE',
          details: `Filtered and paginated return purchase invoices retrieved with status false.`,
        },
      });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub), // L'ID de l'utilisateur qui a effectué l'action
          action: 'GET_ALL_RETURN_PURCHASE_INVOICE',
          details: `Filtered and paginated return purchase invoices retrieved with status true.`,
        },
      });
...
const getSingleReturnPurchaseInvoice = async (req, res) => {
  try {
    ...
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub), // L'ID de l'utilisateur qui a effectué l'action
        action: 'GET_SINGLE_RETURN_PURCHASE_INVOICE',
        entityId: Number(req.params.id),
        entityType: 'returnPurchaseInvoice',
        details: `Return purchase invoice #${req.params.id} retrieved.`,
      },
    });
...
const updateSingleReturnPurchaseInvoice = async (req, res) => {
  try {
    ...
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub), // L'ID de l'utilisateur qui a effectué l'action
        action: 'UPDATE_RETURN_PURCHASE_INVOICE',
        entityId: Number(req.params.id),
        entityType: 'returnPurchaseInvoice',
        details: `Return purchase invoice #${req.params.id} updated.`,
      },
    });
...
const deleteSingleReturnPurchaseInvoice = async (req, res) => {
  try {
    ...
    const [deletePurchaseInvoice, supplier, transaction] =
      ...
      data: {
        userId: Number(req.auth.sub), // L'ID de l'utilisateur qui a effectué l'action
        action: 'DELETE_RETURN_PURCHASE_INVOICE',
        entityId: Number(req.params.id),
        entityType: 'returnPurchaseInvoice',
        details: `Return purchase invoice #${req.params.id} deleted.`,
      },
    });
