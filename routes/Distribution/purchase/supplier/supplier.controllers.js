...
const createSingleSupplier = async (req, res) => {
  if (req.query.query === "deletemany") {
    try {
      ...
      // Log the delete many suppliers action
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'DELETE_MANY_SUPPLIERS',
          details: `Deleted suppliers with IDs ${req.body.join(", ")}`,
        },
      });
    ...
    try {
      ...
      // Log the create many suppliers action
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'CREATE_MANY_SUPPLIERS',
          details: `Created ${createdSupplier.count} suppliers`,
        },
      });
    ...
    try {
      ...
      // Log the create single supplier action
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'CREATE_SINGLE_SUPPLIER',
          details: `Created supplier with name ${req.body.name}`,
        },
      });
...
const getAllSupplier = async (req, res) => {
  if (req.query.query === "all") {
    try {
      ...
      // Log the get all suppliers action
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_ALL_SUPPLIERS',
          details: 'Retrieved all suppliers',
        },
      });
    ...
    try {
      ...
      // Log the get inactive suppliers action
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_INACTIVE_SUPPLIERS',
          details: `Retrieved inactive suppliers with skip ${skip} and limit ${limit}`,
        },
      });
    ...
    try {
      ...
      // Log the get suppliers info action
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_SUPPLIERS_INFO',
          details: 'Retrieved suppliers info',
        },
      });
    ...
    try {
      ...
      // Log the get paginated suppliers action
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub),
          action: 'GET_PAGINATED_SUPPLIERS',
          details: `Retrieved paginated suppliers, showing ${limit} items starting from ${skip === 0 ? 'the first' : `item ${skip + 1}`}.`,
...
const getSingleSupplier = async (req, res) => {
  try {
    ...
    // Log the get single supplier action
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub),
        action: 'GET_SINGLE_SUPPLIER',
        details: `Retrieved supplier with ID ${req.params.id}`,
      },
    });
...
const updateSingleSupplier = async (req, res) => {
  try {
    ...
    // Log the update single supplier action
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub),
        action: 'UPDATE_SINGLE_SUPPLIER',
...
const deleteSingleSupplier = async (req, res) => {
  try {
    ...
    // Log the delete single supplier action
    await prisma.auditLog.create({
      data: {
        userId: Number(req.auth.sub),
        action: 'DELETE_SINGLE_SUPPLIER',
