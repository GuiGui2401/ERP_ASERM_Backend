...
const sendEmail = async (req, res) => {
    ...
    if (req.query.type === "saleinvoice") {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          details: `Request to send sale invoice email for invoice ID ${id} to ${emailaddress}`,
        },
      });
  ...
  <body>
    ...
    // Log the email sent details
      await prisma.auditLog.create({
        data: {
          userId: Number(req.auth.sub), // Assuming Number(req.auth.sub) || null or null if not logged in
          action: 'EMAIL_SENT',
          details: `Sent sale invoice email for invoice ID ${id} to ${emailaddress}`,
        },
      });
